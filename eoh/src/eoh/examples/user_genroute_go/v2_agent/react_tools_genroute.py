...
def analyze_latest_results() -> dict:
    ...
    if _last_evolution_instance:
        ...
        problem = prob_genroute_go.Evaluation(max_instances=1)
        output_base = os.path.join(example_root, "results_genroute_v2")
...
def run_evolution(
    ...
    importlib.reload(prob_genroute_go)
    problem_instance = prob_genroute_go.Evaluation(
    ...
    paras = Paras()
    paras.exp_output_path = os.path.join(example_root, "results_genroute_v2")

    if seed_path and os.path.exists(seed_path):
        seed = os.path.abspath(seed_path)
    else:
        seed = os.path.abspath(os.path.join(example_root, "seeds_genroute_go.json"))
...
def run_code_review(code: str = "", candidate_code: str = "", seed_path: str = "", seed_index: int = 0) -> str:
    ...
    if not code:
        return "FAILED\nNo code provided."
    eva = prob_genroute_go.Evaluation(sim_time_multi=1000000, max_instances=1, run_timeout_s=60)
    fitness = eva.evaluate(code)
    if getattr(eva, "_last_error", None):
        return "FAILED\n" + str(eva._last_error) + "\n" + str(eva._last_traceback or "")
    return f"OK fitness={float(fitness):.6f}"

def add_new_seed(algorithm: str = "", code: str = "", candidate_code: str = "", seed_file: str = "seeds_genroute_go.json") -> str:
    if not code and candidate_code:
        code = candidate_code
    if not algorithm:
        algorithm = "New seed"
    if "func" not in code or "GenRoute" not in code:
        return "Invalid code: must contain GenRoute method definition."
    seed_path = os.path.abspath(os.path.join(example_root, seed_file))
    seeds = []
...
def run_deep_analysis() -> str:
    ...
    if none_rate is not None and float(none_rate) > 0:
        out.append("Diagnosis: Many candidates failed. Prefer compile-safe GenRoute and run run_code_review before add_new_seed.")
    if best is not None and float(best) >= 206.41536:
        out.append("Diagnosis: Stagnation at baseline. GenRoute may have limited leverage; consider pivoting to InsertShips/Optimization/RoutingTS.")
    return "\n".join(out)

def run_comprehensive_evaluation(sim_time_multi: int = 1000000) -> str:
    stats = analyze_latest_results()
    best_code = stats.get("best_code", "")
    if not best_code:
        return "No best_code found. Run evolution first."

    eva = prob_genroute_go.Evaluation(sim_time_multi=int(sim_time_multi), max_instances=8, run_timeout_s=120)
    ...
    report = "# GenRoute Comprehensive Evaluation\n\n"
...
def write_report(sim_time_multi: int = 1000000) -> str:
    md = run_comprehensive_evaluation(sim_time_multi=int(sim_time_multi))
...
def generate_seeds_from_research(query: str, out_name: str = "seeds_genroute_go_research.json") -> str:
    summary = web_search(query)
    seed_src = os.path.abspath(os.path.join(example_root, "seeds_genroute_go.json"))
    if not os.path.exists(seed_src):
        return "Seed source not found: seeds_genroute_go.json"
    ...
    extra = [
        ...
        {
            "algorithm": "Research-guided seed: keep NextTime=TimeCurrent when StationsLen==0. " + alg_note,
        ...
        {
            "algorithm": "Research-guided seed: if Cost<0 then set safe NextSta=0, NextTime=TimeCurrent. " + alg_note,
            "code": "func (assign *Assign) GenRoute() {\n    assign.GenSeq()\n    if assign.StationsLen > 0{\n        if assign.StaIndexesLen < MAXSHIPS {\n            route := RoutingTS(&assign.RoutingTask)\n            assign.RoutingResult = route\n            if assign.Cost >= 0 {\n                assign.NextSta = assign.Route[0].CurSta\n                assign.NextTime = assign.Route[0].CurTime\n            } else {\n                assign.NextSta = 0\n                assign.NextTime = assign.TimeCurrent\n            }\n        } else {\n            assign.Cost = -1\n            assign.NextSta = 0\n            assign.NextTime = assign.TimeCurrent\n        }\n    } else {\n        assign.Cost = 0\n        assign.NextSta = -1\n        assign.NextTime = assign.TimeCurrent\n    }\n}\n",
        },
    ]
