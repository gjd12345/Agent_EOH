...
from prompts_genroute_go import GetPrompts
...
def _replace_genroute(main_go_path: str, new_method: str) -> None:
    with open(main_go_path, "r", encoding="utf-8") as f:
        s = f.read()
    pat = r"func\s*\(\s*assign\s*\*\s*Assign\s*\)\s*GenRoute\s*\(\s*\)\s*\{[\s\S]*?\n\}"
    m = re.search(pat, s)
    if not m:
        raise ValueError("GenRoute method not found in main.go")
...
class Evaluation:
    ...
    def _build_and_run(self, genroute_method_go: str) -> float | None:
        tmp = tempfile.mkdtemp(prefix="eoh_genroute_go_")
        try:
            ...
            _replace_genroute(main_go_path, genroute_method_go)
    ...
    def evaluate(self, code_string):
        ...
        try:
            with warnings.catch_warnings():
                ...
                if "func" not in code_string or "GenRoute" not in code_string:
                    self._last_error = "Missing GenRoute method definition"
